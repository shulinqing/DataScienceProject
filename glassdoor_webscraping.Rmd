...
Perl<- rep(NA,1000)
Excel<- rep(NA,1000)
deeplearning<- rep(NA,1000)
matlab<- rep(NA,1000)
modeling<- rep(NA,1000)
machinelearning<- rep(NA,1000)
hive <- rep(NA,1000)
...
for(j in job.urls){
  ...
  Tableau[index] <- any(grepl("\\bTableau\\b", desc, ignore.case=TRUE))
   ...
   deeplearning[index] <- any(grepl("\\bdeep learning\\b", desc, ignore.case=TRUE))
   matlab[index] <- any(grepl("\\bmatlab\\b", desc, ignore.case=TRUE))
  modeling[index] <- any(grepl("\\bstatistical modeling\\b", desc, ignore.case=TRUE))
  machinelearning[index] <- any(grepl("\\bmachine learning\\b", desc, ignore.case=TRUE))
...
mydf <-  data.frame(company = company_name[1:index],city = jobcity[1:index],state = jobstate[1:index],jobtitle = jobtitle[1:index],industry = industry[1:index], companysize = company_size[1:index],python = python[1:index],R = R[1:index],SAS = SAS[1:index],SQL = SQL[1:index],JAVA = Java[1:index],Tableau = Tableau[1:index],C = C[1:index], Perl = Perl[1:index],Excel = Excel[1:index],DeepLearning = deeplearning[1:index],MATLAB = matlab[1:index],StatisticalModeling = modeling[1:index],MachineLearning  = machinelearning[1:index],HIVE = hive[1:index])
...
mydf$DeepLearning = ifelse(mydf$DeepLearning == "TRUE",1,0)
mydf$MATLAB = ifelse(mydf$MATLAB == "TRUE",1,0)
mydf$StatisticalModeling = ifelse(mydf$StatisticalModeling == "TRUE",1,0)
mydf$C = ifelse(mydf$C == "TRUE",1,0)
mydf$MachineLearning = ifelse(mydf$MachineLearning == "TRUE",1,0)
...
#####test######
url = "https://www.glassdoor.com/Job/data-science-jobs-SRCH_KO0,12_IP2.htm"
fields <- url %>% read_html() %>% html_nodes(".jl")
x <- fields %>% html_nodes(".jobLink") %>% html_attrs()
job.urls <- paste0("https://www.glassdoor.com",
                  unname(unlist(sapply(x, function(x) if(x["class"]=="jobLink") x["href"]))))
...
x2 = "https://www.glassdoor.com/job-listing/data-scientist-monsanto-company-JV_IC1131270_KO0,14_KE15,31.htm?jl=2466042782&ctt=1507507147139"
sourcefile <- suppressWarnings(readLines(x2))
    ...
    current_industry <- sourcefile[str_detect(sourcefile, "\'industry\'")] %>%
      str_extract("\"(.*)\"") %>%  ## Characters that must be quoted to be treated as literals
      str_sub(2, -2)
industry <- if(length(current_industry)>0){current_industry}else{NA}

      size <- sourcefile[str_detect(sourcefile, "\'size\'")] %>% str_extract("\"(.*)\"")%>% str_sub(2, -2)
      size <- gsub("--1","+",size)
   company_size <- if(length(size)>0){size}else{NA}
#str_detect returns true or false
#str_sub returns a string
#str_extract returns an extracted string
